...
async function detectProjectType(cwd: string, context: ProjectContext): Promise<void> {
  ...
  if (await fileExists(goModPath)) {
    ...
    if (moduleMatch) context.name = moduleMatch[1].trim();
    context.rawConfig = content;
    return;
  }
  ...
  if (await fileExists(pomPath)) {
    ...
    if (artifactMatch) context.name = artifactMatch[1];
  ...
  if (hasGitHubActions) {
    ...
    try {
      ...
      for (let i = 0; i < yamlFiles.length; i++) {
        context.cicd.workflows.push(yamlFiles[i].replace(/\.ya?ml$/, ''));
