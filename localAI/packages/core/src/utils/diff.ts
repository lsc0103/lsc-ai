    ...
    if (isChange) {
      ...
      if (!currentHunk) {
        ...
        for (let c = contextStart; c < idx; c++) {
          const ctx = changes[c];
          if (ctx.type === 'equal' && ctx.oldIdx !== undefined) {
      ...
      if (nextChangeIdx === -1 || nextChangeIdx - idx > contextLines * 2) {
        ...
        currentHunk = null;
      } else if (change.oldIdx !== undefined) {
        // 继续 hunk
        currentHunk.lines.push({
          type: 'context',
          content: oldLines[change.oldIdx],
...
export function formatUnifiedDiff(
  ...
  const { oldFileName = 'a', newFileName = 'b', showLineNumbers = true } = options || {};
  const lines: string[] = [];
  ...
  for (const hunk of diff.hunks) {
    ...
    for (const line of hunk.lines) {
      ...
      switch (line.type) {
        case 'add':
          prefix = '+';
          if (showLineNumbers && line.newLineNum) {
            lineNum = `${line.newLineNum.toString().padStart(4)}  `;
          }
          break;
        case 'remove':
          prefix = '-';
          if (showLineNumbers && line.oldLineNum) {
