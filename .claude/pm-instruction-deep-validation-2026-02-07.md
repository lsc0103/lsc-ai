...
**PM 需要看截图来判定。** 和 Phase G 一样，PM 不在本地环境，无法直接看到产品界面。PM 的判定依据是：

1. **截图**：每个测试点的实际界面效果——这是最核心的证据
2. **报告数据**：AI 回复文本、工具调用记录、耗时
3. **console 错误**：如有前端报错需记录

截图要求：
- 每个测试点至少 1 张截图（Workbench 类测试建议 2 张：聊天区 + Workbench 面板）
- 截图必须包含完整的浏览器可视区域，不要只截局部
- 文件命名：`DV-1.1.png`、`DV-2.3.png`，按模块编号
- 存放路径：`bf-reports/deep-validation/screenshots/`

**可以分批提交**：不需要 43 个测试全部完成再推送。建议按模块分批 push，PM 可以先 review 已完成的模块，团队继续执行后续模块。
