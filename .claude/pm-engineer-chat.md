...
### [工程师] 2026-02-12 — Sprint 2 RAG 知识库 MVP 完成 + Embedding 方案建议

#### S2 开发完成

Sprint 2 RAG 知识库 MVP 已全栈实现（commit `a3c8b03`），4 个 Agent 并行开发：

| 任务 | 交付物 | 行数 |
|------|--------|------|
| S2-T1 CRUD API | knowledge.module/controller/service.ts — 8 个 API + JWT 认证 + MinIO 集成 | ~400 |
| S2-T2 文档 Pipeline | document-pipeline.service.ts + text-extractor.ts + text-chunker.ts | ~550 |
| S2-T3 RAG 检索 | rag.service.ts + rag-tools.ts + knowledge-search.controller.ts — searchKnowledge Mastra 工具 | ~350 |
| S2-T4 前端 UI | Knowledge.tsx + KnowledgeDetail.tsx + knowledge-api.ts + 路由/侧边栏/ChatInput 修改 | ~760 |
| **总计** | **19 个文件** | **+2263 行** |

Server + Web 双包 tsc --noEmit 编译通过。

功能闭环：**上传 → MinIO存储 → 文本提取(txt/md/pdf/docx/xlsx) → 三级分块 → 向量化 → LibSQLVector存储 → 语义搜索 → AI对话自动调用 → 前端管理界面**

#### 🔴 重要发现：公司已有 Embedding API + Rerank API

检查公司 API 资产时发现两个关键服务：

| 服务 | 端点 | 用途 |
|------|------|------|
| **text-embedding-v1/v2** | `10.18.55.233:30069/embedding/chi/v1/embeddings` | 文本向量化 |
| **text-rerank-v1/v2** | `10.18.55.233:30069/rerank/chi/v1/rerank` | 搜索结果精排 |

**当前实现用的是本地 fastembed（all-MiniLM-L6-v2 英文模型，384维），中文效果有限。**

对比分析：

| 维度 | 公司 text-embedding API | 本地 fastembed |
|------|------------------------|---------------|
| 中文效果 | **优秀**（大概率是通义千问 text-embedding-v2，中文专用，1536维） | 一般（英文模型对中文语义理解有限） |
| 部署负担 | 零（已部署在 GPU 集群） | 需占用服务器 CPU + ~1GB 内存 |
| 速度 | <100ms（内网 API） | 200-500ms/batch（CPU 推理） |
| 离线可用 | 需内网连通 | 完全离线 |

**建议方案**：
1. **生产环境切换到公司 Embedding API** — 中文效果碾压 fastembed，零部署负担
2. **加入 Rerank 精排** — 先 Embedding 粗筛 top20 → Rerank 精排 top5，搜索精度提升 20-40%
3. **fastembed 保留为开发/离线 fallback**
4. **实现方式**：参照 LLM ModelFactory 模式，创建 EmbeddingFactory，通过环境变量切换 Provider

代码改动预估 ~80 行（新建 embedding-factory.ts + 修改 pipeline/rag 两个 service）。

#### 请 PM 决策

1. **是否批准将 S2 的 Embedding 切换为公司 API？**（涉及 .env 中添加公司内网地址配置）
2. **是否在 S2 中加入 Rerank 能力？**（额外 ~50 行代码）
3. **S2 代码 Review 是否通过？是否有 R 项需要工程评估？**
...
## 第七节 — S3 PM 二审指令（2026-02-12 签发）

### 背景

总工程师已完成第一轮代码审查并修复了 5 项问题（越权访问、鉴权缺失、数据格式）。PM 在此基础上对 S3 全部交付物进行了深度审查，发现 **4 个 P0 + 7 个 P1 + 6 个 P2** 问题。

**S3 判定：有条件通过。** 核心功能已具备，但存在安全漏洞和功能缺失，不允许进入 S4 前遗留 P0。
...
| # | 修复内容 | PM 判定 |
|---|---------|--------|
| 1 | knowledge-search.controller 增加 `@UseGuards(JwtAuthGuard)` | ✅ 有效 |
| 2 | project.service `findById/update/delete` 增加 userId 所有权校验 + 404/403 | ✅ 有效 |
| 3 | user.controller `update` 非管理员只能改自己，不能改 status | ✅ 有效 |
| 4 | user.service `mapUserRoles()` 统一扁平化 roles 数组 | ✅ 有效 |
| 5 | `USER_SELECT` 补充 `lastLoginAt` 字段 | ✅ 有效 |

**PM 确认：总工程师修复质量合格，全部采纳。**

---

### 二、P0 修复指令（4 项，阻塞 S3 关闭）

> **红线：以下 4 项全部修复并通过验证前，S3 不允许关闭，S4 不允许启动。**

#### P0-R1：Settings 页面缺少修改密码功能

**现状**：Settings.tsx 页面只有偏好开关（全部禁用），用户无法修改自己的密码。这是企业级平台的安全红线。

**要求**：
1. Settings 页面新增"修改密码"区域
2. 必须包含三个字段：当前密码、新密码、确认密码
3. 后端新增 `PATCH /api/users/change-password` 端点
4. 后端必须验证当前密码正确后才允许修改
5. 新密码必须 ≥8 位，且与当前密码不同
6. 修改成功后前端显示成功提示

**验收标准**：用户能够在 Settings 页面成功修改密码，用新密码重新登录成功。

#### P0-R2：`GET /api/users/:id` 未限制访问权限

**现状**：总工程师修了 `update`，但没修 `findOne`。任何已登录用户调用 `GET /api/users/任意ID` 都能获取该用户的完整信息（含邮箱、创建时间、角色列表、最后登录时间）。

**要求**：
1. 普通用户只能查看自己的详情（`id === req.user.id`）
2. 管理员可查看任意用户详情
3. 非法访问返回 `403 Forbidden`

**验收标准**：普通用户请求他人 ID 返回 403。管理员请求任意 ID 正常返回。

#### P0-R3：知识库搜索 API 无所属权校验

**现状**：总工程师加了 JWT 鉴权（解决了未登录访问），但已登录用户仍可搜索任意知识库。这在多用户环境下等于**数据泄露**。

**要求**：
1. `POST /api/knowledge-bases/:id/search` 必须验证该知识库属于当前用户
2. `POST /api/knowledge-bases/search`（全局搜索）必须只搜索当前用户拥有的知识库
3. 非法访问返回 `403 Forbidden`

**验收标准**：用户 A 搜索用户 B 的知识库返回 403。用户 A 全局搜索只返回自己知识库的结果。

#### P0-R4：前端所有 catch 块为空（吞错误）

**现状**：审查发现 `Users.tsx`、`Roles.tsx`、`Settings.tsx`、`Projects.tsx` 等管理页面中，所有 API 调用的 catch 块都是 `catch (e) {}`——操作失败时用户看不到任何反馈，以为操作成功了。

**要求**：
1. 所有 catch 块必须调用 `message.error('具体错误描述')` 或等效的用户反馈机制
2. 错误信息必须有意义：「创建用户失败」「删除角色失败」「修改密码失败」等
3. 如果后端返回了具体错误信息（如"用户名已存在"），前端必须展示

**验收标准**：在任意管理页面执行一个必然失败的操作（如创建重复用户名），用户看到明确的错误提示。

---

### 三、P1 修复指令（7 项，S4 中必须完成）

| # | 问题 | 要求 |
|---|------|------|
| **P1-R5** | 后端无 DTO 输入校验 | 所有 POST/PATCH 端点使用 class-validator DTO：密码 ≥8 位、邮箱格式、用户名 3-30 字符、分页 page≥1 / pageSize 1-100 |
| **P1-R6** | `findByUsername()` 返回含密码 hash | 该方法仅在 auth.service 登录时使用，需添加注释标注"仅限认证流程调用"，或拆分为 `findByUsernameWithPassword()` 明确命名 |
| **P1-R7** | 硬删除无审计 | 用户/角色/项目删除时，向 AuditLog 表写入操作记录（who, what, when）。DB 已有该表，未使用 |
| **P1-R8** | 用户创建无密码确认 | 管理员创建用户的表单增加"确认密码"字段，前端校验两次输入一致 |
| **P1-R9** | AdminRoute 未检查 isAuthenticated | token 过期时 `user` 可能为 null/undefined，需先检查认证状态再检查角色 |
| **P1-R10** | 角色页面无搜索和分页 | 当角色数量超过 20 时需要搜索和分页功能 |
| **P1-R11** | 角色管理页面无权限分配 UI | 后端 `permissions[]` 字段已存在但前端完全没有管理入口。至少实现权限列表勾选 |
...
| # | 问题 | 备注 |
|---|------|------|
| P2-R12 | Project 搜索只匹配 name 不匹配 description | 扩展为 OR 搜索 |
| P2-R13 | 项目工作目录无路径合法性校验 | 防止 `../../` 路径穿越 |
| P2-R14 | Settings 偏好开关全部禁用无说明 | 要么实现功能，要么移除 UI |
| P2-R15 | 被禁用用户 JWT 未过期仍可操作 | 请求拦截中校验 user.status |
| P2-R16 | `loginFailCount` 存在但未实现锁定 | N 次失败后锁定账号 |
| P2-R17 | 知识库搜索内部错误信息直接返回前端 | 服务端记日志，前端返通用提示 |

---

### 五、工作安排

**阶段 1（P0 修复）**：4 项 P0 全部修复 → 提交 → PM 逐项验收

**验收方式**：
- P0-R1：截图 Settings 页面 + 实际修改密码 + 新密码登录
- P0-R2：用普通用户 token 请求他人 ID，截图 403 响应
- P0-R3：用用户 A token 搜索用户 B 知识库，截图 403 响应
- P0-R4：触发一个失败操作，截图前端错误提示

**阶段 2（P1 修复）**：纳入 S4 Sprint 计划，与 S4 新功能并行

---

### 六、纪律要求

1. **不允许自行降级 P0**。如果认为某项 P0 不合理，向 PM 提出书面理由，由 PM 决定是否降级
2. **不允许以"后续优化"为理由跳过 P0**。P0 是上线阻塞项，不是建议
3. **修复后必须自测**。提交前工程师必须自行验证修复有效，不要把明显 bug 推给 PM 验收
4. **catch 块不允许为空**。这是基本的工程素养，不是产品要求。从今以后所有新代码的 catch 块必须有用户反馈

---

**签发人**：PM
**签发时间**：2026-02-12
**状态**：等待工程团队执行阶段 1（P0 修复）

---

### [总工程师] 2026-02-12 — S3 二审 P0 修复报告 + 测试能力升级通知
...
| P0 | 问题 | 修复方案 | 自测 |
|-----|------|---------|------|
| **R1** | 设置页无法修改密码 | 全栈实现：`UserService.changePassword()` 含旧密码验证+新密码不能重复+bcrypt hash；`UserController` PATCH `/users/change-password`（路由在 `:id` 之前防冲突）；`Settings.tsx` 修改密码卡片（3字段+前端校验）；`userApi.changePassword()` | tsc ✓ |
| **R2** | GET /users/:id 越权 | `UserController.findOne()` 添加 `isAdmin \|\| id === req.user.id` 检查，否则 403 | tsc ✓ |
| **R3** | 知识库搜索无隔离 | 单库搜索：验证 `kb.userId === req.user.id`；全局搜索：`findMany({ where: { userId } })` 限定用户 KB，逐库搜索合并排序 | tsc ✓ |
| **R4** | 7处空 catch 静默 | `Users.tsx`(2) `Roles.tsx`(2) `Projects.tsx` `ProjectDetail.tsx` `Knowledge.tsx` 统一模式：`if (!error?.errorFields) message.error(...)` 区分表单校验和 API 错误 | tsc ✓ |

**编译验证**：Server + Web 双包 `tsc --noEmit` 零错误通过。

请 PM 按验收方式逐项确认。P1(7项) + P2(6项) 按指示纳入 S4。

---

#### 二、测试能力升级通知：Claude in Chrome

**向 PM 报告一项重大能力升级。**

工程团队现已接入 **Claude in Chrome** 浏览器自动化能力。这意味着总工程师现在同时具备两种测试手段：

| 能力 | Playwright（已有） | Claude in Chrome（新增） |
|------|-------------------|------------------------|
| **测试方式** | 脚本化自动测试 | 真实浏览器操作（模拟真人用户） |
| **交互方式** | 代码选择器定位元素 | 视觉识别 + 语义查找元素 |
| **适用场景** | 回归测试、批量验证、CI/CD | 场景验收、UX 体验验证、探索性测试 |
| **验证深度** | 断言式（pass/fail） | 可截图、可录 GIF、可读页面内容 |
| **覆盖范围** | 预定义路径 | 可像真人一样自由探索 |

**具体能力：**
- 可在真实 Chrome 浏览器中导航、点击、输入、滚动
- 可截取屏幕截图和录制操作 GIF
- 可读取页面 DOM 树、查找交互元素、读取控制台日志和网络请求
- 可执行 JavaScript 读取页面状态
- 可模拟完整的用户操作流程（登录→操作→验证结果）

**对 PM 的意义：**

1. **PM 验收方式升级**：PM 提出的验收标准（如"截图 Settings 页面 + 实际修改密码 + 新密码登录"），总工程师可以直接在真实浏览器中模拟执行，产出截图/GIF 作为验收证据
2. **场景测试更真实**：不再局限于 Playwright 脚本的选择器匹配，可以像真人用户一样操作产品，发现 UX 层面的问题
3. **双轨验证**：Playwright 负责批量回归（确保不退化），Chrome 负责场景验收（确保好用），两者互补
4. **组建测试团队**：可以同时派出多个 Agent 进行并行浏览器测试，大幅提升测试效率

**建议**：从 S3 P0 验收开始，总工程师可以使用 Chrome 自动化产出 PM 要求的验收截图。PM 是否同意采用此方式？
