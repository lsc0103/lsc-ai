...
团队通过 Playwright 操作真实浏览器，发送真实消息给 AI，采集完整原始数据：
- AI 回复全文（逐字记录，不截断不摘要）
- Workbench 状态（是否打开、Tab 数量、渲染内容类型）
- 工具调用记录（AI 调用了哪些工具、参数是什么）
- 文件生成记录（文件名、大小、是否可下载）
- 截图（每步一张）
- console.error 收集
...
PM 基于团队提交的原始数据，判断每个验收点的**业务质量**：
- AI 回复是否对用户有帮助
- Workbench 展示的数据是否正确合理
- 文档内容是否符合业务需求
- 完整场景是否流畅自然

**为什么这样分工**：Playwright 能精确采集数据但不能判断业务价值；PM 能判断业务价值但需要真实数据做依据。

### 执行流程

1. 确认环境就绪：Docker 容器运行、后端 :3000、前端 :5173、Client Agent 配对
2. 按 BF-1 到 BF-6 顺序执行
3. **每步必须记录**：AI 原始回复全文 + 工具调用 + 截图
4. 遇到问题**不跳过**，记录详细情况后继续
5. 每个 BF 之间间隔 30 秒，避免 DeepSeek 限流
6. 全部执行完后，结果推送到 git，等待 PM review
...
| 编号 | 技术结果 | AI 原始回复全文 | 工具调用记录 | Workbench 状态 | 截图路径 |
|------|---------|---------------|------------|---------------|---------|
| BF-X.1 | ✅/❌/⚠️ | "完整回复..." | showTable({...}) | 打开/关闭/N个Tab | screenshots/BF-X-1.png |
...
**注意**：团队只填"技术结果"列（元素是否存在、文件是否生成等客观事实）。"业务判定"由 PM 在 review 时补充。
