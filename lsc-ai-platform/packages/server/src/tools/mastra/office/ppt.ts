...
function parseMarkdownToSlides(markdown: string): { title?: string; slides: SlideContent[] } {
  ...
  for (const line of lines) {
    ...
    if (h3Match && h3Match[1] && currentSlide) {
    ...
    if (imageMatch && imageMatch[2] && currentSlide) {
    ...
    if (nativeChartMatch && nativeChartMatch[1] && currentSlide) {
      try {
    ...
    if (listMatch && listMatch[1] && currentSlide) {
    ...
    if (indentedMatch && indentedMatch[1] && currentSlide) {
      if (!currentSlide.bullets) currentSlide.bullets = [];
    ...
    if (orderedMatch && orderedMatch[1] && orderedMatch[2] && currentSlide) {
      if (!currentSlide.bullets) currentSlide.bullets = [];
