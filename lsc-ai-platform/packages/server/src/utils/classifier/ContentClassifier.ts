...
export class ContentClassifier implements IContentClassifier {
  ...
  private generateThinkingSummary(content: string): string {
    ...
    const first = firstLine ? firstLine.slice(0, 50) : '';
    const last = lastLine ? lastLine.slice(0, 50) : '';
  ...
  static summarizeThinking(chunks: ClassifiedContent[]): string {
    ...
    // 只显示首尾和数量提示
    const firstChunk = thinkingChunks[0];
    const lastChunk = thinkingChunks[thinkingChunks.length - 1];
    const first = firstChunk ? firstChunk.content.slice(0, 50) : '';
    const last = lastChunk ? lastChunk.content.slice(-50) : '';
